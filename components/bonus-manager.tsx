            ...
            visibleBonuses.map((bonus) => (
              ...
              >
                ...
                <div className="grid grid-cols-[1fr_auto] items-stretch gap-2 text-sm md:min-w-[210px]">
                  <div className="rounded-xl bg-emerald-50 px-3 py-2 md:text-right">
              ...
              <div className="flex max-h-[calc(100vh-1.5rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[rgba(121,150,118,0.28)] bg-white shadow-xl shadow-stone-900/10 sm:max-h-[calc(100vh-3rem)]">
                ...
                <div className="overflow-y-auto px-5 py-4">
                  ...
                  <form action={deleteBonusAction} className="mt-3" onSubmit={(event) => { if (!confirm("Delete this bonus? This cannot be undone.")) event.preventDefault(); }}>
                    <input type="hidden" name="bonusId" value={selected.id} />
                    <button className="w-full rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 hover:bg-rose-100">Delete Bonus</button>
                  </form>
                </div>
              </div>
            </div>,
